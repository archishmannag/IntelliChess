...
dialog::dialog(std::string heading_text, std::string dialog_text, sf::Vector2f position)
{
    ...
    // Wrap the text to only have 40 characters per line.
...
{
    // TODO: Add click on dialog text to copy to clipboard
      ...
      cancel_callback_(cancel_callback)
{
    ...
    input_text_.setString("_"); // <- Cursor
...
void input_dialog::event_handler(sf::Event &event)
{
    ...
    else if (event.type == sf::Event::TextEntered)
    {
        ...
        else if (event.text.unicode >= 32 /* ' ' */ && event.text.unicode <= 126 /* '~' */) // The ASCII printable characters
        {
