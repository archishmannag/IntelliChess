...
void tile_block::set_tile_rect_scale(float x, float y)
{
...
/**
 * @brief Helper functions for printing player info to console
 *
 */
namespace
{
...
void game_board::init()
{
    ...
    /* All the callback fnctions for the menu bar */
    {
        ...
        call_backs.exit_ = [this]()
        { window_->close(); };
        call_backs.new_game_ = [this]()
        { undo_all_moves(); };
        call_backs.undo_move_ = [this]()
        { undo_last_move(); };
        call_backs.evaluate_board_ = [this]()
        {
            std::cout << standard_board_evaluator::evaluation_details(*board_, game_setup_->get_search_depth()) << std::endl;
        };
        call_backs.current_state_ = [this]()
        {
    ...
    // Smooth textures do not pixelate when scaled
...
void game_board::scale_board()
{
    ...
    {
        tile_block.set_tile_rect_scale(min, min);
        ...
        legal_moves.erase(
            std::remove_if(
                ...
                [this](std::shared_ptr<move> m)
                {
					...
					return true; }),
            legal_moves.end());
        for (const std::shared_ptr<move> &m : legal_moves)
...
void game_board::render_tile_blocks()
{
    for (tile_block &tile_block : tile_blocks_)
    {
        ...
        {
            // Draw piece
            const std::shared_ptr<piece> &piece = board_->get_tile(tile_block.get_tile_id())->get_piece_on_tile();
...
void game_board::ai_move_generator::run()
{
    ...
    do
    {
        // Render and update the window while the AI is calculating the move
        // Check the other thread every 15 milliseconds to see if it's done
