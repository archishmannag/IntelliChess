/**
 * @file WhitePlayer.cpp
 * @author Archishman Nag (nag.archishman@gmail.com)
 * @brief Implementation of the white player class
 * @version 1.1.0
 *
 */
    ...
    : player(b)
{
    ...
    checked_ = !calculate_attacks_on_tile(player_king_->get_piece_position(), bslm).empty(); // No attacks on king = safe
...
std::vector<std::shared_ptr<move> > white_player::calculate_king_castles(const std::vector<std::shared_ptr<move> > &ol) const
{
    ...
    if (player_king_->is_first_move() && !is_in_check() && !is_castled() && is_castle_capable())
    {
        ...
        if (!board_.lock()->get_tile(61)->is_tile_occupied() && !board_.lock()->get_tile(62)->is_tile_occupied())
        {
            ...
            {
                if (calculate_attacks_on_tile(61, ol).empty() && calculate_attacks_on_tile(62, ol).empty() && !board_utils::pawn_castle_attack_check(board_.lock().get(), player_king_.get(), 52))
                    king_castles.push_back(std::make_shared<king_side_castle_move>(board_.lock(), player_king_, 62, std::dynamic_pointer_cast<rook>(rook_tile->get_piece_on_tile()), rook_tile->get_tile_coordinate(), 61));
            }
            else
        ...
        if (!board_.lock()->get_tile(59)->is_tile_occupied() && !board_.lock()->get_tile(58)->is_tile_occupied() && !board_.lock()->get_tile(57)->is_tile_occupied())
        {
            ...
            if (rook_tile->is_tile_occupied() && rook_tile->get_piece_on_tile()->is_first_move() && rook_tile->get_piece_on_tile()->get_piece_type() == piece_type::rook)
            {
                if (calculate_attacks_on_tile(59, ol).empty() && calculate_attacks_on_tile(58, ol).empty() && calculate_attacks_on_tile(57, ol).empty() && !board_utils::pawn_castle_attack_check(board_.lock().get(), player_king_.get(), 52))
                    king_castles.push_back(std::make_shared<queen_side_castle_move>(board_.lock(), player_king_, 58, std::dynamic_pointer_cast<rook>(rook_tile->get_piece_on_tile()), rook_tile->get_tile_coordinate(), 59));
            }
            else
