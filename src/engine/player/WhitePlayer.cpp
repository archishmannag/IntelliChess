        ...
        is_castle_capable())
    {
            ...
            !board_.lock()->get_tile(62)->is_tile_occupied())
        {
            ...
            if (rook_tile->is_tile_occupied() &&
                rook_tile->get_piece_on_tile()->is_first_move() &&
                rook_tile->get_piece_on_tile()->get_piece_type() == piece_type::rook)
            {
                if (calculate_attacks_on_tile(61, ol).empty() &&
                    calculate_attacks_on_tile(62, ol).empty() &&
                    !board_utils::pawn_castle_attack_check(board_.lock().get(), player_king_.get(), 52))
