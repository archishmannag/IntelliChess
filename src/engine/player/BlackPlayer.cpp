...
std::vector<std::shared_ptr<move>> black_player::calculate_king_castles(const std::vector<std::shared_ptr<move>> &ol) const
{
        ...
        is_castle_capable())
    {
        ...
        if (is_king_side_castle_capable() &&
            !board_.lock()->get_tile(5)->is_tile_occupied() &&
            !board_.lock()->get_tile(6)->is_tile_occupied())
        {
            ...
            {
                if (calculate_attacks_on_tile(5, ol).empty() &&
                    calculate_attacks_on_tile(6, ol).empty() &&
                    !board_utils::pawn_castle_attack_check(board_.lock().get(), player_king_.get(), 12))
            ...
            !board_.lock()->get_tile(1)->is_tile_occupied())
        {
            ...
            if (rook_tile->is_tile_occupied() &&
                ...
                rook_tile->get_piece_on_tile()->get_piece_type() == piece_type::rook)
            {
                if (calculate_attacks_on_tile(3, ol).empty() &&
